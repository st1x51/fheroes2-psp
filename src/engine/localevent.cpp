...
SceCtrlData pad;
SceCtrlData oldpad;
static int mouse_key_mod=0;
static int mx, my;

#define MOUSE_BUTTON_RIGHT PSP_CTRL_CIRCLE
#define MOUSE_BUTTON_LEFT  PSP_CTRL_CROSS
#define MOUSE_WHEEL_UP     PSP_CTRL_RTRIGGER
#define	MOUSE_WHEEL_DOWN   PSP_CTRL_LTRIGGER


bool LocalEvent::pspKeyPress(int pspkey)
{
     if(pad.Buttons & pspkey && !(oldpad.Buttons & pspkey)) //down
		 return true;
	 return false;
}

bool LocalEvent::pspKeyRelease(int pspkey)
{
	if(!(pad.Buttons & pspkey) && oldpad.Buttons & pspkey) //up
         return true;
	 return false;
}

void LocalEvent::pspSetKey(int pspkey, KeySym gamekey)
{
	if(pspKeyPress(pspkey)) //down
	{
       SetModes(KEY_PRESSED);
       key_value = gamekey;
	}

	if(pspKeyRelease(pspkey)) //up
	{
...
bool LocalEvent::HandleEvents(bool delay)
{
	...
	pspSetKey(PSP_CTRL_UP,    KEY_UP   );
	pspSetKey(PSP_CTRL_DOWN,  KEY_DOWN );
	pspSetKey(PSP_CTRL_RIGHT, KEY_RIGHT);
	pspSetKey(PSP_CTRL_LEFT,  KEY_LEFT );
    pspSetKey(PSP_CTRL_SQUARE,    KEY_SPACE  );
	pspSetKey(PSP_CTRL_TRIANGLE,  KEY_ESCAPE );
    ...
    //printf("x: %i y: %i\n",mx,my);
    if((mouse_cu.x != mx) || (mouse_cu.y != my))
	...
	{
		if(pspKeyPress(MOUSE_BUTTON_LEFT)) //down
		{
	       SetModes(MOUSE_PRESSED);
	       mouse_pl = mouse_cu;
	       SetModes(CLICK_LEFT);
	       mouse_key_mod = MOUSE_BUTTON_LEFT;
		}
		if(pspKeyRelease(MOUSE_BUTTON_LEFT)) //up
		{
	       mouse_rl = mouse_cu;
	       mouse_key_mod = MOUSE_BUTTON_LEFT;
	...
	{
		if(pspKeyPress(MOUSE_BUTTON_RIGHT)) //down
		{
	       SetModes(MOUSE_PRESSED);
	       mouse_pr = mouse_cu;
	       SetModes(CLICK_RIGHT);
	       mouse_key_mod = MOUSE_BUTTON_RIGHT;
		}

		if(pspKeyRelease(MOUSE_BUTTON_RIGHT)) //up
		{
	       mouse_rr = mouse_cu;
	       mouse_key_mod = MOUSE_BUTTON_RIGHT;
	       ResetModes(MOUSE_PRESSED);
	...
	{
		if(pspKeyPress(MOUSE_WHEEL_UP)) //down
		{
	       SetModes(MOUSE_PRESSED);
	       mouse_pm = mouse_cu;
	       mouse_key_mod = MOUSE_WHEEL_UP;
		}
		if(pspKeyRelease(MOUSE_WHEEL_UP)) //up
		{
	       mouse_rm = mouse_cu;
	       mouse_key_mod = MOUSE_WHEEL_UP;
	       ResetModes(MOUSE_PRESSED);
	...
	{
		if(pspKeyPress(MOUSE_WHEEL_DOWN)) //down
		{
	       SetModes(MOUSE_PRESSED);
	       mouse_pm = mouse_cu;
	       mouse_key_mod = MOUSE_WHEEL_DOWN;
		}
		if(pspKeyRelease(MOUSE_WHEEL_DOWN)) //up
		{
	       mouse_rm = mouse_cu;
	       mouse_key_mod = MOUSE_WHEEL_DOWN;
	       ResetModes(MOUSE_PRESSED);
...
{
    return (modes & MOUSE_PRESSED) && mouse_key_mod == MOUSE_BUTTON_LEFT/*SDL_BUTTON_LEFT == mouse_button*/;
}
...
{
    return (modes & MOUSE_PRESSED) && mouse_key_mod == MOUSE_BUTTON_RIGHT/*SDL_BUTTON_RIGHT == mouse_button*/;
}
...
{
    return (modes & MOUSE_PRESSED) && mouse_key_mod == MOUSE_WHEEL_UP /*SDL_BUTTON_WHEELUP == mouse_button*/;
}

bool LocalEvent::MouseWheelDn(void) const
{
    return (modes & MOUSE_PRESSED) && mouse_key_mod == MOUSE_WHEEL_DOWN /*SDL_BUTTON_WHEELDOWN == mouse_button*/;
}
...
#endif
{
    ...
    if(le.redraw_cursor_func)
	{
	    if(le.modes & MOUSE_OFFSET)
    		(*(le.redraw_cursor_func))(mx + le.mouse_st.x, my + le.mouse_st.y);
    	    else
		(*(le.redraw_cursor_func))(mx, my);
	}
...
{
    return SDL_ENABLE; // SDL_EventState(type, SDL_QUERY);
}
